...
import { PropertyFaq } from "@/components/properties/property-faq";
import {
  PropertyDocuments,
  PropertyFees,
  PropertyIncome,
  ...
  PropertyRisks,
  PropertyTenant,
} from "@/components/properties/property-disclosures";
...
export async function generateMetadata({
  ...
  const property = await withDbRetry(() => getPropertyBySlug(slug));
  ...
  return {
    // The layout's template appends "| GharShare", so the title must not.
    title: property.title,
    description,
    alternates: { canonical: `/properties/${property.slug}` },
    openGraph: {
      type: "article",
      title: `${property.title} — ${property.shortLocation}`,
      description,
      url: `/properties/${property.slug}`,
      // The card image itself is generated per-listing by the sibling
...
export default async function PropertyDetailPage({
  ...
  /**
   * Draft mode is how the console previews an unpublished listing.
   *
   * Only the admin who went through /admin/properties/preview has the
   * cookie, and Next bypasses the ISR cache for that request alone — every
   * other visitor still gets the cached public page, and still cannot see a
   * draft at all.
   */
  const { isEnabled: isDraft } = await draftMode();

  const property = await withDbRetry(() =>
    getPropertyBySlug(slug, { includeUnpublished: isDraft })
  );
  ...
  const showDraftBanner = isDraft && !property.isPublished;
  ...
  const similarProperties = await withDbRetry(() =>
    getSimilarProperties(property, 3)
  );

  return (
    <div className="mx-auto max-w-7xl px-4 py-10 pb-24 sm:px-6 lg:px-8 lg:pb-10">
      {/* Unmistakable, because the whole risk of a preview is forgetting it
          is one and assuming the listing is live. */}
      {showDraftBanner && (
        <p
          role="status"
          className="mb-6 rounded-lg border border-gold bg-gold-light px-4 py-3 text-sm text-gold-700"
        >
          <strong className="font-medium">Draft preview.</strong> This listing
          is not published — nobody else can see this page.
        </p>
      )}
      ...
      <div className="mt-6 grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,1fr)_380px]">
        <div>
          ...
          <div className="mt-8">
            ...
            {/* Share sits under the title on a phone and beside it from sm.
                Inline at 390px it stole ~90px from a 3xl serif headline and
                forced the property name to wrap badly. */}
            <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              ...
              {/* Property research in India spreads over WhatsApp. Without
                  this, a listing only ever reaches the one person who found it. */}
          ...
          {isFractional && (
            <div className="mt-8 rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-7">
              ...
              <p className="mt-4 border-t border-border pt-4 text-xs leading-relaxed text-muted-foreground">
                ...
                property-specific documentation and the full{" "}
                <Link
          ...
          {/* The questions that actually decide a six-figure commitment, in
              the order somebody asks them: what does it earn, who pays it,
              what does it cost me, what can I read, what could go wrong,
              where is it and who runs it. Each renders only if it has data. */}
          <PropertyIncome property={property} />
          <PropertyTenant property={property} />
          <PropertyFees property={property} />
          <PropertyDocuments property={property} />
          <PropertyRisks property={property} />
      ...
      {/* RealEstateListing is the type Google understands for a property page;
          BreadcrumbList produces the Home › Properties › … trail in results
          instead of a bare URL. Both describe only what is on the page. */}
